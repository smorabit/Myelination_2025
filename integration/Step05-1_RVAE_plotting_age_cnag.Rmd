...
seurat_subset_both <- hdWGCNA:::BinPseudotime(seurat_subset_both, n_bins=c(10,50,100))
...
# link_df_full <- read.csv(file=paste0("/home/groups/singlecell/smorabito/PhD_ongoing/AMRF/processed/", cur_celltype, '_', cur_group, '_full_link_table.csv'))
...
# pdf(paste0(fig_dir,  cur_group, '_cluster_timepoint_proportions.pdf'), width=6, height=4)
# prop_tp / prop_ct
# dev.off()
...
plot_df <- reshape2::melt(as.matrix(ordered[rev(rownames(ordered)[rownames(ordered) %in% switch_genes]),]))
...
Not run and probably not going to run on CNAG cluster 
